...
INTEGER :: ios,record !,recordinf, recordsup !pour lecture table
...
INTEGER :: kTmat, idtype, idbande,Nmoments
...
IF(CCLOUD=="LIMT") THEN
    Hydromet_const_file='../param/LIMA_constants.txt'
END IF
...
DO idbande=2,2
  ...
  IF (idbande .EQ. 4) bande='K' !Ka
  IF (idbande .EQ. 5) bande='W' 
...
!  IF (idtype .EQ. 11) typeh='cc' ! cloud water (for LIMA : no sea/land distinction)
  
  WRITE(0,*) '---- Hydrometeor type: ',typeh
  
  IF (typeh=='ii' .OR. ((CCLOUD=='LIMA' .OR. CCLOUD=='LIMT') .AND. (typeh=='rr' .or. &
          typeh=='mm' .or. typeh=='tt'))) THEN ! .or. typeh=='cc'))) THEN
      Nmoments=2
  ELSE
      Nmoments=1
  ENDIF
  ...
  DO while(espece /= typeh) 
    ...
    print*,espece,aj,bj,alphaj,nuj,Xj,Cj,ccj,ddj
  ENDDO
  close(333)
  ...
  nLines=nTc*nELEV*nFw*nD ! number of lines of the Tmat file
                               ! (without the 3 first description lines)
                               
  WRITE(0,*) "nD=",nD, " nELEV=",nELEV," nTc=",nTc,"nFw=",nFw
  ...
  5003 FORMAT ('Tc;ELEV;P3;M;RR;' &
               'sigvv;sighh;REdeltaco;IMdeltaco;' &
               'zhh;zdr;kdp;rhohv;Ah;Av;' &
               'sigvvR;sighhR;REdeltacoR;IMdeltacoR;' &
